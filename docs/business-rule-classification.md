...
| BR-CART-004 | Cart can contain items from only one restaurant | Cross-layer Rule | Cart add-item behavior and checkout restaurant identity validation | Cart protects item consistency; CheckoutDomainService also verifies the loaded Restaurant matches Cart.RestaurantId. |
