...
- [x] T045 [US3] Create `Talabat.Delivery.API.Tests/DeliveriesAuthorizationTests.cs` — test matrix: no token → 401, wrong scope → 403, wrong role → 403, valid token → passes authorization, anonymous → 401 (FR-014 scenarios: scope enforcement, role enforcement, anonymous rejection)
...
- [x] T048 [US3] Create `Talabat.Customer.API.Tests/AuthorizationTests.cs` — test matrix: no token → 401, wrong scope → 403, correct scope (no role) → passes, correct scope + role → passes, anonymous catalog → 200, profile creation with no role → not 403 (FR-014 scenarios: scope-only policy, profile creation bypass, anonymous catalog access)
